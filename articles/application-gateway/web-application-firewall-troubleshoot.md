...
Stellen Sie zunächst sicher, dass Sie die [WAF-Übersicht](waf-overview.md) und die Dokumente zur [WAF-Konfiguration](application-gateway-waf-configuration.md) gelesen haben. Vergewissern Sie sich außerdem, dass Sie die [WAF-Überwachung](application-gateway-diagnostics.md) aktiviert haben. In diesen Artikeln wird die Funktionsweise der WAF und der WAF-Regelsätze sowie der Zugriff auf WAF-Protokolle erläutert.
...
![Ausschlüsse](media/waf-tshoot/exclusion-list.png)
...
![WAF-Ausschlusslisten](media/waf-tshoot/waf-config.png)
...
![WAF-Regeln](media/waf-tshoot/waf-rules.png)
...
![WAF-Ausschluss](media/waf-tshoot/waf-exclusion-02.png)
...
Aufgrund Ihrer Kenntnisse über die Funktionsweise der CRS-Regelsätze und da Ihnen bekannt ist, dass der Regelsatz CRS 3.0 mit einem System für die Anomaliebewertung arbeitet (siehe [Web Application Firewall für Azure Application Gateway](waf-overview.md)), wissen Sie, dass die beiden unteren Regeln mit der Eigenschaft **action: Blocked** auf Grundlage der Gesamtbewertung der Anomalie blockiert werden. Die Regeln, auf die Sie sich konzentrieren müssen, sind die beiden ersten Regeln.
