...
1. Sie müssen das Metadatenzertifikat in F5 (Kerberos) importieren. Es wird später im Setupvorgang verwendet. Navigieren Sie zu **System > Certificate Management > Traffic Certificate Management > SSL Certificate List** (System > Zertifikatverwaltung > Zertifikatverwaltung für Datenverkehr > SSL-Zertifikatliste). Klicken Sie in der rechten Ecke auf **Import** (Importieren).

    ![F5-Konfiguration (Kerberos)](./media/kerbf5-tutorial/configure01.png) 

1. Darüber hinaus muss für den Hostnamen (`Kerbapp.superdemo.live`) ein **SSL-Zertifikat** angegeben werden. In diesem Beispiel wird ein Platzhalterzertifikat verwendet.

    ![F5-Konfiguration (Kerberos)](./media/kerbf5-tutorial/configure02.png) 
 
1. Klicken Sie in F5 auf **Access > Guided Configuration > Federation > SAML Service Provider** (Zugriff > Interaktive Konfiguration > Verbund > SAML-Dienstanbieter).
...
1. Erstellen Sie einen neuen virtuellen Server, und geben Sie einen Wert für **Destination Address** (Zieladresse) ein. Wählen Sie das **Platzhalterzertifikat** (oder das für die Anwendung hochgeladene **Zertifikat**) aus, das Sie zuvor hochgeladen haben. Wählen Sie anschließend unter **Associated Private Key** (Zugeordneter privater Schlüssel) einen Wert aus.
...
1. Wählen Sie unter **Single Sign-on Settings** (Einstellungen für einmaliges Anmelden) die Option **Kerberos** und anschließend **Advanced Settings** (Erweiterte Einstellungen) aus. Die Anforderung muss in `user@domain.suffix` erstellt werden.

1. Geben Sie unter **username source** (Quelle des Benutzernamens) den Namen `session.saml.last.attr.name. http://schemas.xmlsoap.org/ws/2005/05/identity/claims/givenname` ein. Eine vollständige Liste der Variablen und Werte finden Sie im Anhang.
...
1. Eine ausführliche Dokumentation zu den Eigenschaften für Endpunktüberprüfungen (**Endpoint Check Properties**) erhalten Sie vom [Supportteam für den F5-Client (Kerberos)](https://support.f5.com/csp/knowledge-center/software/BIG-IP?module=BIG-IP%20APM45).

    ![F5-Konfiguration (Kerberos)](./media/kerbf5-tutorial/configure10.png) 

1. Eine ausführliche Dokumentation zu den Eigenschaften für die Sitzungsverwaltung (**Session Management Properties**) erhalten Sie vom [Supportteam für den F5-Client (Kerberos)](https://support.f5.com/csp/knowledge-center/software/BIG-IP?module=BIG-IP%20APM45).
