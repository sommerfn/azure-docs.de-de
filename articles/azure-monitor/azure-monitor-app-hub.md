---
layout: HubPage
hide_bc: false
title: Dokumentation zu Azure Monitor Application Insights – Tutorials, API-Referenz | Microsoft-Dokumentation
description: Azure Monitor Application Insights bietet eine umfassende Überwachung der Anwendungsleistung auf Codeebene.
ms.service: application-insights
ms.subservice: ''
ms.topic: hub-page
author: mrbullwinkle
ms.author: mbullwin
ms.date: 01/15/2019
ms.openlocfilehash: 254cd4e67c0c64ba9b21133e957ae2cacfefe7a7
ms.sourcegitcommit: b4f201a633775fee96c7e13e176946f6e0e5dd85
ms.translationtype: HT
ms.contentlocale: de-DE
ms.lasthandoff: 10/18/2019
ms.locfileid: "72596817"
---
<div id="main" class="v2">
    <div class="container">
        <h1>Dokumentation zu Azure Monitor Application Insights</h1>
        <p>Azure Monitor Application Insights bietet eine umfassende Überwachung der Anwendungsleistung auf Codeebene.</p>
        <hr style="margin: 30px 0;" />
        <ul class="pivots">
            <li>
                <a href="#products"></a>
                <ul id="products">
                    <li>
                        <a class="singlePanelNavItem selected" style="display: none" href="#indexA" data-linktype="self-bookmark"></a>
                        <ul class="panelContent singlePanelContent" id="indexA" style="border: medium; border-image: none; margin-top: 0px; display: flex; float: left;">
                            <li class="fullSpan">
                                <a href="#index1" data-linktype="self-bookmark"></a>
                                <ul class="cardsF cols cols4" id="index1" style="float: left; display: flex; width: 100%; border-bottom: 1px var(--grey-lighter) solid;">  
                                    <li>
                                        <ul class="cardsB panelContent" id="cardtypes-B" style="float: left; display: flex; width: 100%;">
                                            <li>
                                               <!-- <a href="">-->
                                                   <div class="cardSize">
                                                        <div class="cardPadding">
                                                            <div class="card">
                                                                <div class="cardImageOuter">
                                                                    <div class="cardImage">
                                                                        <img alt="" src="https://docs.microsoft.com/media/common/i_learn-about.svg" data-linktype="external">
                                                                    </div>
                                                                </div>
                                                                <div class="cardText" style="padding-left: 0px">
                                                                    <h3>Grundlagen</h3>
                                                                    <p>
                                                                        <a href="./app/app-insights-overview.md">Was ist Application Insights?</a><br>
                                                                    </p>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </a>
                                            </li>
                                            <li>
                                           </li>
                                        </ul>
                                    </li>
                                    <li>
                                        <div class="cardSize">
                                            <div class="cardPadding">
                                                <div class="card">
                                                    <div class="cardText">
                                                        <h3>Neuerungen</h3>
                                                        <p>
                                                            <a href="./app/azure-vm-vmss-apps.md">Überwachung für Anwendungen auf virtuellen Azure-Computern und Azure-VM-Skalierungsgruppen ohne Code</a><br>
                                                            <a href="./app/kubernetes.md">Überwachen von Kubernetes-Anwendungen</a><br>
                                                            <a href="./app/status-monitor-v2-overview.md">Statusmonitor v2 für IIS-Apps</a><br>
                                                        </p>
                                                        <br><br>
                                                        <h3>Schnellstarts</h3>
                                                        <p>
                                                            <a href="./learn/quick-monitor-portal.md">ASP.NET</a><br>
                                                            <a href="./learn/dotnetcore-quick-start.md">ASP.NET Core</a><br>
                                                            <a href="./learn/nodejs-quick-start.md">Node.js</a><br>
                                                            <a href="./learn/mobile-center-quickstart.md">Mobile</a><br>
                                                            <a href="./app/website-monitoring.md">Browser-/clientseitige Überwachung</a>
                                                        </p>
                                                        <br><br>
                                                        <h3>Lernprogramme</h3>
                                                        <p>
                                                            <a href="./learn/tutorial-runtime-exceptions.md">Suchen nach und Diagnostizieren von Laufzeitausnahmen</a><br>
                                                            <a href="./learn/tutorial-performance.md">Suchen nach und Diagnostizieren von Leistungsproblemen</a><br>
                                                            <a href="./learn/tutorial-alert.md">Überwachen der Anwendungsintegrität und Ausgeben von Warnungen</a><br>
                                                            <a href="./learn/tutorial-users.md">Verständnis des Benutzerverhaltens</a><br>
                                                            <a href="./learn/tutorial-app-dashboards.md">Erstellen benutzerdefinierter Dashboards</a>
                                                            </p>
                                                        <br><br>
                                                        <h3>Verteilte Ablaufverfolgung</h3>
                                                        <p>
                                                            <a href="./app/distributed-tracing.md">Was ist die verteilte Ablaufverfolgung?</a><br>
                                                            <a href="./app/opencensus-python.md">OpenCensus für Python</a><br>
                                                            <a href="./app/opencensus-go.md">OpenCensus für Go</a><br>
                                                            <a href="./app/opencensus-local-forwarder.md">Lokale Weiterleitung</a>
                                                            </p>
                                                        <br><br>
                                                        <h3>Automatische intelligente Erkennung</h3>
                                                        <p>
                                                            <a href="./app/proactive-failure-diagnostics.md">Anomalien bei Fehlern</a><br>
                                                            <a href="./app/proactive-performance-diagnostics.md">Leistungsanomalien</a><br>
                                                            <a href="./app/proactive-trace-severity.md">Verschlechterung der Schweregrade bei der Ablaufverfolgung</a><br>
                                                            <a href="./app/proactive-exception-volume.md">Ungewöhnliche Zunahme von Ausnahmen</a><br>
                                                            <a href="./app/proactive-potential-memory-leak.md">Arbeitsspeicherverluste</a><br>
                                                            <a href="./app/proactive-low-utilization-cloud-resources.md">Niedrige CPU-Auslastung</a><br>
                                                            <a href="./app/proactive-application-security-detection-pack.md">Sicherheitsprobleme</a><br>
                                                            <a href="./app/proactive-cloud-services.md">Azure Cloud Services</a><br>
                                                            <a href="./app/proactive-arm-config.md">Verwalten der intelligenten Erkennung mit Vorlagen</a>
                                                        </p>
                                                        <br><br>
                                                        <h3>Verweis</h3>
                                                        <p>
                                                            <a href="./app/auto-collect-dependencies.md">Automatisches Sammeln von Abhängigkeiten</a><br>
                                                            <a href="./app/correlation.md">Telemetriekorrelation</a><br>
                                                            <a href="./app/nuget.md">NuGet-Pakete</a><br>
                                                            <a href="./app/azure-functions-supported-features.md">Azure Functions</a>
                                                            </p>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </li>
                                    <li>
                                        <div class="cardSize">
                                            <div class="cardPadding">
                                                <div class="card">
                                                    <div class="cardText">
                                                        <h3>Java-Anwendungen</h3>
                                                        <p>
                                                            <a href="./app/java-get-started.md">Umfassende Überwachungskonfiguration</a><br>
                                                            <a href="https://docs.microsoft.com/java/azure/spring-framework/configure-spring-boot-java-applicationinsights?view=azure-java-stable">Spring Boot Starter</a><br>
                                                            <a href="./app/micrometer-java.md">Micrometer-Metriken</a><br>
                                                            <a href="./app/java-filter-telemetry.md">Filtern von Telemetriedaten</a><br>
                                                            <a href="./app/java-trace-logs.md">Ablaufverfolgungsprotokollierung</a><br>
                                                            <a href="./app/java-agent.md">Agentkonfiguration für Abhängigkeitsnachverfolgung</a><br>
                                                            <a href="./app/api-custom-events-metrics.md">Benutzerdefinierte Ereignisse und Metriken</a><br>
                                                            <a href="https://docs.microsoft.com/azure/azure-monitor/app/configuration-with-applicationinsights-config#channel-parameters-java
">Config.xml</a><br>
                                                            <a href="https://github.com/Microsoft/ApplicationInsights-Java">Open Source SDK für Java – GitHub-Repository</a>
                                                        </p>
                                                        <br><br>
                                                        <h3>.NET-Anwendungen</h3>
                                                        <p>
                                                            <a href="./app/asp-net.md">Umfassende Überwachungskonfiguration</a><br>
                                                            <a href="./app/monitor-performance-live-website-now.md">Statusmonitor</a><br>
                                                            <a href="./app/azure-web-apps.md">App Service-Überwachung</a><br>
                                                            <a href="./app/azure-vm-vmss-apps.md">Überwachung von Anwendungen auf VMs und VM-Skalierungsgruppen</a><br>
                                                            <a href="./app/api-custom-events-metrics.md">Benutzerdefinierte Ereignisse und Metriken</a><br>
                                                            <a href="https://github.com/Microsoft/ApplicationInsights-dotnet">Open Source SDK für .NET – GitHub-Repository</a>
                                                        </p>
                                                        <br><br>
                                                          <h3>.NET Core-Anwendungen</h3>
                                                        <p>
                                                            <a href="./app/asp-net-core.md">Umfassende Überwachungskonfiguration</a><br>
                                                            <a href="./app/profiler-aspnetcore-linux.md">ASP.NET Core-Profilerstellung</a><br>
                                                            <a href="./app/api-custom-events-metrics.md">Benutzerdefinierte Ereignisse und Metriken</a><br>
                                                            <a href="https://github.com/Microsoft/ApplicationInsights-aspnetcore">Open Source SDK für .NET Core – GitHub-Repository</a>
                                                        </p>
                                                        <br><br>
                                                          <h3>Node.js-Anwendungen</h3>
                                                        <p>
                                                            <a href="./app/nodejs.md">Umfassende Überwachungskonfiguration</a><br>
                                                            <a href="./app/api-custom-events-metrics.md">Benutzerdefinierte Ereignisse und Metriken</a><br>
                                                            <a href="https://github.com/Microsoft/ApplicationInsights-node.js">Open Source SDK für Node.js – GitHub-Repository</a>
                                                        </p>
                                                        <br><br>
                                                        <h3>Client-/browserseitige Überwachung</h3>
                                                        <p>
                                                            <a href="./app/javascript.md">Umfassende Überwachungskonfiguration</a><br>
                                                            <a href="./app/api-custom-events-metrics.md">Benutzerdefinierte Ereignisse und Metriken</a><br>
                                                            <a href="https://github.com/Microsoft/ApplicationInsights-JS">Open Source SDK für JavaScript – GitHub-Repository</a><br>
                                                        </p>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </li>
                                    <li>
                                        <div class="cardSize">
                                            <div class="cardPadding">
                                                <div class="card">
                                                    <div class="cardText">
                                                        <h3>Analyse des Benutzerverhaltens</h3>
                                                        <p>
                                                            <a href="./app/usage-overview.md">Übersicht</a><br>
                                                            <a href="./app/usage-send-user-context.md">Senden von Benutzerkontext-IDs</a><br>
                                                            <a href="./app/usage-segmentation.md">Benutzer, Sitzungen und Ereignisse</a><br>
                                                            <a href="./app/usage-funnels.md">Trichter</a><br>
                                                            <a href="./app/usage-cohorts.md">Kohorten</a><br>
                                                            <a href="./app/usage-impact.md">Auswirkung</a><br>
                                                            <a href="./app/usage-retention.md">Vermerkdauer</a><br>
                                                            <a href="./app/usage-flows.md">Benutzerflows</a>
                                                        </p>
                                                        <br><br>
                                                        <h3>Überwachung in Echtzeit,<br> Visualisierung und Analyse</h3>
                                                        <p>
                                                            <a href="./app/overview-dashboard.md">Übersichtsdashboard und Anwendungsdashboard</a><br>
                                                            <a href="./app/monitor-web-app-availability.md">Überwachung der globalen Verfügbarkeit</a><br>
                                                            <a href="./app/live-stream.md">Live Metrics Stream</a><br>
                                                            <a href="./app/app-map.md">Verteilte App-Zuordnung</a><br>
                                                            <a href="../monitoring-and-diagnostics/resource-group-insights.md">Überwachung auf Ressourcengruppenebene</a><br>
                                                            <a href="./app/usage-workbooks.md">Interaktive Berichterstellung</a><br>
                                                            <a href="./app/export-power-bi.md">Power BI-Integration</a><br>
                                                            <a href="https://docs.microsoft.com/azure/monitoring-and-diagnostics/monitoring-metric-charts">Untersuchen von Metriken</a>
                                                        </p>
                                                        <br><br>
                                                        <h3>Datenschutz, Sicherheit, Nutzung und Kosten</h3>
                                                        <p>
                                                            <a href="https://docs.microsoft.com/azure/log-analytics/log-analytics-personal-data-mgmt">Umgang mit personenbezogenen Daten</a><br>
                                                            <a href="./app/data-retention-privacy.md">Datensammlung, -aufbewahrung und&amp; -speicherung</a><br>
                                                            <a href="./app/resources-roles-access-control.md">Rollenbasierter Zugriff</a><br>
                                                            <a href="./app/ip-addresses.md">IP-Adressen und Firewallausnahmen</a><br>
                                                            <a href="https://docs.microsoft.com/azure/monitoring-and-diagnostics/monitoring-usage-and-estimated-costs">Nutzung und geschätzte Kosten</a>
                                                        </p>
                                                        <br><br>
                                                        <h3>Datenmodell</h3>
                                                        <p>
                                                            <a href="./app/data-model.md">Übersicht</a><br>
                                                            <a href="./app/data-model-request-telemetry.md">Anforderung</a><br>
                                                            <a href="./app/data-model-dependency-telemetry.md">Abhängigkeit</a><br>
                                                            <a href="./app/data-model-exception-telemetry.md">Ausnahme</a><br>
                                                            <a href="./app/data-model-trace-telemetry.md">Ablaufverfolgung</a><br>
                                                            <a href="./app/data-model-event-telemetry.md">Event</a><br>
                                                            <a href="./app/data-model-metric-telemetry.md">Metrik</a><br>
                                                            <a href="./app/data-model-context.md">Kontext</a>
                                                            </p>
                                                        </p>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </li>
                                </ul>
                            </li>
                        </ul>
                    </li>
                </ul>
            </li>
        </ul>
    </div>
</div>
