...
* Wenn eine Anwendung eines der Skalierbarkeitsziele überschreiten muss, sollten Sie mehrere Speicherkonten erstellen und die Anwendungsdaten auf mehrere Speicherkonten aufteilen. Konzipieren Sie in diesem Fall die Anwendung so, dass Sie künftig weitere Speicherkonten für den Lastenausgleich hinzufügen können. Zum Redaktionszeitpunkt konnte jedes Azure-Abonnement bis zu 250 Speicherkonten pro Region umfassen (sofern es mit dem Azure Resource Manager-Modell bereitgestellt wurde).  Speicherkonten verursachen keine Kosten außer die Nutzung in Bezug auf gespeicherte Daten, durchgeführte Transaktionen und übertragene Daten.
...
### <a name="subheading47"></a>Namenskonvention für Partitionen

Azure Storage verwendet ein bereichsbasiertes Partitionierungsschema für Skalierung und Lastausgleich des Systems. Der Partitionsschlüssel (Konto + Container + Blob) wird zur Partitionierung von Daten in Bereiche verwendet, und diese Bereiche werden per Lastenausgleich über das System verteilt. Dies bedeutet, dass Namenskonventionen wie die lexikalische Sortierung (zum Beispiel *mypayroll*, *myperformance*, *myemployees* usw.) oder die Verwendung von Zeitstempeln (*log20160101*, *log20160102*, *log20160102* usw.) sich für die Partitionen eignen, die sich auf demselben Partitionsserver befinden können, bis sie durch den Lastenausgleich in kleinere Bereiche aufgeteilt werden. So können z. B. alle Blobs in einem Container als ein einziger Server behandelt werden, bis die Last dieser Blobs einen weiteren Ausgleich der Partitionsbereiche erfordert. Ebenso kann eine Gruppe leicht geladener Konten, deren Namen lexikalisch sortiert sind, von einem einzigen Server bedient werden, bis die Last auf einem oder allen diesen Konten deren Aufteilung auf mehrere Partitionsserver erfordert. Jeder Lastausgleichsvorgang kann die Latenz der Speicheraufrufe während des Vorgangs beeinflussen. Die Fähigkeit des Systems, eine plötzliche Zunahme des Datenverkehrs auf einer Partition zu bewältigen, wird durch die Skalierbarkeit eines einzelnen Partitionsservers begrenzt, bis der Lastenausgleich wirksam wird und den Partitionsschlüsselbereich ausgleicht.

Sie können die Zahl dieser Vorgänge mit einigen bewährten Vorgehensweisen reduzieren.  

* Verwenden Sie möglichst größere „Put Blob“- oder „Put Block“-Größen (über 4 MiB für Standardkonten und über 256 KiB für Premium-Konten), um HTBB (High-Throughput Block Blob) zu aktivieren. HTBB unterstützt eine leistungsfähige, von Partitionsnamen unabhängige Datenerfassung.
* Untersuchen Sie genau die Benennungskonvention, die Sie für Konten, Containers, Blobs, Tabellen und Warteschlangen verwenden. Erwägen Sie, Konto-, Container- oder Blobnamen mithilfe einer Hashfunktion, die Ihren Anforderungen am besten gerecht wird, mit einem 3-stelligen Hashpräfix zu versehen.  
* Wenn Sie Ihre Daten mit Zeitstempeln oder numerischen Bezeichnern organisieren, müssen Sie sicherstellen, dass Sie keine Nur-anfügen- oder Nur-voranstellen-Verkehrsmuster verwenden. Diese Muster eignen sich nicht für ein bereichsbasiertes Partitionierungssystem und könnten dazu führen, dass der gesamte Datenverkehr zu einer einzigen Partition geleitet und so das System an einem wirksamen Lastausgleich gehindert wird. Wenn z. B. in täglichen Vorgängen ein Blobobjekt mit einem Zeitstempel wie *yyyymmdd* nutzen, wird der gesamte Datenverkehr für diesen täglichen Vorgang an ein einziges Objekt geleitet, das von einem einzigen Partitionsserver bedient wird. Prüfen Sie, ob die Grenzwerte pro Blob und pro Partition Ihren Bedürfnissen entsprechen, und erwägen Sie, diesen Vorgang bei Bedarf in mehrere Blobs aufzuteilen. Auch wenn Sie Zeitreihendaten in Ihren Tabellen speichern, könnte der gesamte Verkehr an den letzten Teil des Schlüssel-Namespace weitergeleitet werden. Wenn Sie Zeitstempel oder numerische IDs verwenden müssen, versehen Sie die ID mit einem 3-stelligen Hashpräfix, oder stellen Sie bei Zeitstempeln den Sekundenteil als Präfix voran, z. B. *ssyyyymmdd*. Wenn Auflistungs- und Abfragevorgänge routinemäßig ausgeführt werden, wählen Sie eine Hashfunktion, die die Anzahl der Abfragen begrenzt. In anderen Fällen ist möglicherweise ein zufälliges Präfix ausreichend.  
...
Weitere Informationen zu SAS finden Sie unter [Gewähren von eingeschränktem Zugriff auf Azure Storage-Ressourcen mithilfe von SAS (Shared Access Signature)](storage-sas-overview.md).  
...
#### <a name="subheading10"></a>Erhöhen der minimalen Anzahl von Threads bei Verwendung von synchronem Code mit asynchronen Aufgaben

Mit diesem Code wird die Mindestanzahl von Threads im Threadpool erhöht:  
...
#### <a name="subheading11"></a>Nutzung der Garbage Collection von .NET 4.5 und höher
...
#### <a name="subheading14"></a>Fehler durch Drosselung oder ausgelasteten Server
...
Zusätzlich zu den zuvor beschriebenen bewährten Vorgehensweisen für [Alle Dienste](#allservices) können die folgenden bewährten Vorgehensweisen speziell für den Blob-Dienst angewendet werden.  

### <a name="blob-specific-scalability-targets"></a>Blobspezifische Skalierbarkeitsziele
...
Um Blobs schnell hochzuladen, müssen Sie erst die folgende Frage beantworten: Laden Sie ein Blob oder mehrere hoch?  Befolgen Sie die Anleitung unten, um die richtige Methode für Ihr Szenario auszuwählen.  
...
* Node.js: Verwenden Sie „parallelOperationThreadCount“ entweder für die Anforderungsoptionen oder für den Blob-Dienst.
...
#### <a name="subheading26"></a>Deaktivieren des Nagle-Algorithmus
...
### <a name="subheading39"></a>Skalierungsgrenzen
...
### <a name="subheading40"></a>Deaktivieren des Nagle-Algorithmus
...
### <a name="subheading44"></a>Aktualisieren von Nachrichten

Sie können den **UpdateMessage**-Vorgang verwenden, um das Unsichtbarkeits-Zeitlimit zu erhöhen oder die Statusinformationen einer Nachricht zu aktualisieren. Obwohl diese Option wirkungsvoll ist, sollten Sie beachten, dass jeder **UpdateMessage**-Vorgang auf das Skalierbarkeitsziel angerechnet wird. Dies kann jedoch sehr viel effizienter sein als ein Workflow, der einen Auftrag von einer Warteschlange zur nächsten verschiebt, wenn der jeweilige Schritt abgeschlossen ist. Mithilfe des **UpdateMessage**-Vorgangs können der Auftragsstatus von Ihrer Anwendung in der Nachricht gespeichert und die Ausführung fortgesetzt werden. So muss die Nachricht nach Abschluss eines Schritts für den nächsten Auftragsschritt nicht jedes Mal erneut in die Warteschlange eingereiht werden.  
