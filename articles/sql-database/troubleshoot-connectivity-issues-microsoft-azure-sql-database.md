...
description: Beheben von Verbindungsproblemen mit Microsoft Azure SQL-Datenbank
...
2. Wenn keine bekannten Ausfälle angezeigt werden, navigieren Sie zur [Microsoft Azure-Supportwebsite](http://azure.microsoft.com/support/options), um eine Supportanfrage zu öffnen.
...
Das Problem tritt auf, weil die Anwendung nicht in der Lage ist, eine Verbindung mit dem Server herzustellen.

Um dieses Problem zu beheben, führen Sie die im Abschnitt mit dem Titel **Schritte zum Beheben häufiger Verbindungsprobleme** genannten Schritte (in der angegebenen Reihenfolge) aus.
...
Diese Probleme treten auf, weil die Anwendung nicht in der Lage ist, eine Verbindung mit dem Server herzustellen.
...
Um dieses Problem zu beheben, [konfigurieren Sie die Firewalleinstellungen für SQL-Datenbank mit dem Azure-Portal.](https://docs.microsoft.com/azure/sql-database/sql-database-firewall-configure)
...
## <a name="unable-to-login-to-the-server-errors-18456-40531"></a>Die Anmeldung auf dem Server ist nicht möglich (Fehler 18456, 40531)
...
Um dieses Problem zu beheben, wenden Sie sich an Ihren Dienstadministrator, um einen gültigen SQL-Benutzernamen und ein gültiges Kennwort anzugeben.

In der Regel kann der Dienstadministrator den Anmeldenamen mit den folgenden Schritten hinzufügen:
...
4. Wenn der Benutzername für die SQL-Anmeldung nicht vorhanden ist, muss er mithilfe von SSMS erstellt werden:

   1. Doppelklicken Sie auf **Sicherheit**, um den Bereich zu erweitern. 
...
Weitere Informationen finden Sie unter [SQL-Datenbank-Authentifizierung und -Autorisierung: Gewähren von Zugriff](https://docs.microsoft.com/azure/sql-database/sql-database-manage-logins).

## <a name="connection-timeout-expired-errors"></a>Verbindungstimeoutfehler

### <a name="systemdatasqlclientsqlexception-0x80131904-connection-timeout-expired"></a>System.Data.SqlClient.SqlException (0x80131904): Verbindungstimeoutfehler.

**Detaillierter Fehler**

``System.Data.SqlClient.SqlException (0x80131904): Connection Timeout Expired. The timeout period elapsed while attempting to consume the pre-login handshake acknowledgement. This could be because the pre-login handshake failed or the server was unable to respond back in time. The duration spent while attempting to connect to this server was - [Pre-Login] initialization=3; handshake=29995;``

### <a name="systemdatasqlclientsqlexception-0x80131904-timeout-expired"></a>System.Data.SqlClient.SqlException (0x80131904): Timeout ist abgelaufen.
...
Diese Ausnahmen können aufgrund von Verbindungs- oder Abfrageproblemen auftreten. Um zu bestätigen, dass dieser Fehler auf Verbindungsprobleme zurückzuführen ist, überprüfen Sie den Abschnitt **Bestätigen, ob der Fehler aufgrund eines Verbindungsproblems auftritt**:

Verbindungstimeouts treten auf, weil die Anwendung nicht in der Lage ist, eine Verbindung mit dem Server herzustellen. Um dieses Problem zu beheben, führen Sie die im Abschnitt mit dem Titel **Schritte zum Beheben häufiger Verbindungsprobleme** genannten Schritte (in der angegebenen Reihenfolge) aus.
...
Dieses Problem tritt auf, weil bei einer Neukonfiguration bzw. einem Failover für das Back-End ein vorübergehender Fehler aufgetreten ist.

Um dieses Problem zu beheben, warten Sie kurz, und wiederholen Sie den Vorgang dann. Es ist kein Supportfall erforderlich, wenn das Problem nicht weiterhin besteht.
...
## <a name="connection-terminated-due-to-hitting-some-system-defined-limit"></a>Die Verbindung wurde beendet, weil ein vom System definiertes Limit erreicht wurde
...
* Überprüfen Sie, ob zeitintensive Abfragen ausgeführt werden:
  ...
  > [!NOTE]
  > Dies ist ein minimalistischer Ansatz, der das Problem möglicherweise nicht zwangsläufig löst.

  1. Überprüfen Sie die Sicht [sys.dm_exec_requests](https://docs.microsoft.com/sql/relational-databases/system-dynamic-management-views/sys-dm-exec-requests-transact-sql), um blockierende Anforderungen anzuzeigen, indem Sie die folgende SQL-Abfrage ausführen:
...
* Wenn die Datenbank trotz Berücksichtigung von Blockierungen und zeitintensiven Abfragen konsistent an ihre Grenzen stößt, sollten Sie ein Upgrade auf eine der neuen Vorschau-Editionen (z.B. [Standard- oder Premium-Editionen](https://azure.microsoft.com/pricing/details/sql-database/)) in Betracht ziehen.

Weitere Informationen zu den Preisoptionen für SQL-Datenbank finden Sie unter [Preise für Azure SQL-Datenbank](https://azure.microsoft.com/pricing/details/sql-database/single/).
...
Mithilfe der folgenden Schritte können Sie das Problem umgehen oder zusätzliche Optionen nutzen, die Sie in Erwägung ziehen können.

1. Überprüfen Sie die aktuelle Größe der Datenbank, indem Sie das Dashboard im Azure-Portal verwenden.

   > [!NOTE]
   > Sie können die folgende SQL-Abfrage verwenden, um zu ermitteln, welche Tabellen den meisten Speicherplatz beanspruchen und mögliche Kandidaten für die Bereinigung sind:
...
3. Wenn die Datenbankgröße bereits über die maximal unterstützte Größe für Ihre Edition hinausgeht, können Sie einen der folgenden Schritte ausführen:
   1. Führen Sie normale Datenbankbereinigungsaktivitäten aus (bereinigen Sie unerwünschte Daten mit truncate/delete usw., oder verschieben Sie Daten mithilfe von SSIS, bcp usw.).
   2. Partitionieren oder löschen Sie Daten, löschen Sie Indizes, oder informieren Sie sich in der Dokumentation über mögliche Lösungen. 
...
### <a name="error-40549-session-is-terminated-because-you-have-a-long-running-transaction"></a>Fehler 40549: Die Sitzung wird aufgrund einer Transaktion mit langer Laufzeit beendet.
...
1. Überprüfen Sie die Sicht sys.dm_exec_requests, um alle geöffneten Sitzungen mit einem hohen Wert für die total_elapsed_time-Spalte durch Ausführen des folgenden SQL-Skripts anzuzeigen:

   ```
   SELECT * FROM dm_exec_requests
   ```
2. Ermitteln Sie den Eingabepuffer für die Abfrage, die zeitintensiv ist. 
...
1. Ändern Sie die Abfragen, um die Verwendung des temporären Tabellenspeicherplatzes zu verringern. 
2. Löschen Sie temporäre Objekte, wenn diese nicht mehr benötigt werden. 
3. Kürzen Sie Tabellen, oder entfernen Sie nicht verwendete Tabellen.

### <a name="error-40552-the-session-has-been-terminated-because-of-excessive-transaction-log-space-usage"></a>Fehler 40552: Die Sitzung wurde aufgrund übermäßiger Verwendung des Speicherplatzes für das Transaktionsprotokoll beendet.
...
* Das Problem tritt aufgrund von insert-, update- oder delete-Vorgängen auf. Versuchen Sie, die Anzahl der Zeilen zu verringern, die sofort ausgeführt werden, indem Sie Batchverarbeitung oder eine Aufteilung in mehrere kleinere Transaktionen implementieren.
* Das Problem tritt aufgrund der Indexneuerstellungsvorgänge auf. Stellen Sie sicher, dass Sie die folgende Formel verwenden: Anzahl der Zeilen, die in der Tabelle betroffen sind * (durchschnittliche Größe des aktualisierten Felds in Bytes + 80) < 2 GB

  > [!NOTE]
  > Bei der Indexneuerstellung sollte die durchschnittliche Größe des aktualisierten Felds durch die durchschnittliche Indexgröße ersetzt werden.
...
Dieses Problem tritt auf, weil das Konto nicht über Zugriffsberechtigungen für die Masterdatenbank verfügt. SQL Server Management Studio (SSMS) versucht standardmäßig, eine Verbindung mit der Masterdatenbank herzustellen.

Gehen Sie folgendermaßen vor, um das Problem zu beheben:

1. Klicken Sie auf dem Anmeldebildschirm von SSMS auf **Optionen**, und klicken Sie dann auf **Verbindungseigenschaften**. 
2. Geben Sie unter **Verbindung mit Datenbank herstellen** den Standarddatenbanknamen des Benutzers als standardmäßige Anmeldedatenbank ein, und klicken Sie dann auf **Verbinden**.
...
## <a name="confirm-whether-an-error-is-due-to-a-connectivity-issue"></a>Bestätigen, dass ein Fehler aufgrund eines Verbindungsproblems vorliegt

Um zu bestätigen, dass ein Fehler aufgrund eines Verbindungsproblems vorliegt, überprüfen Sie die Stapelüberwachung für Frames, die Aufrufe zum Öffnen einer Verbindung wie die folgenden anzeigen (beachten Sie den Verweis auf die **SqlConnection**-Klasse):
...
Wenn die Ausnahme aufgrund von Abfrageproblemen auftritt, ähnelt der Aufrufsstapel den folgenden Aufrufstapeln (beachten Sie den Verweis auf die **SqlCommand**-Klasse). In diesen Szenarien müssen Sie Ihre [Abfragen optimieren](http://blogs.msdn.com/b/sqlblog/archive/2013/11/01/is-my-query-running-fine-in-the-cloud.aspx).
...
```
Weitere Anleitungen zum Optimieren der Leistung finden Sie in den folgenden Artikeln:
...
1. Stellen Sie sicher, dass TCP/IP auf dem Anwendungsserver als Clientprotokoll aktiviert ist. Weitere Informationen finden Sie unter [Konfigurieren von Clientprotokollen](https://docs.microsoft.com/sql/database-engine/configure-windows/configure-client-protocols). Überprüfen Sie auf Anwendungsservern, auf denen keine SQL Server-Tools installiert sind, ob TCP/IP aktiviert ist, indem Sie **cliconfg.exe** (Hilfsprogramm für das SQL Server-Clientnetzwerk) ausführen. 
2. Überprüfen Sie die Verbindungszeichenfolge der Anwendung, um sicherzustellen, dass sie ordnungsgemäß konfiguriert ist. Stellen Sie z.B. sicher, dass in der Verbindungszeichenfolge der richtige Port (1433) und der vollqualifizierte Servername angegeben werden.
...
3. Versuchen Sie, das **Verbindungstimeout** zu erhöhen. Microsoft empfiehlt die Verwendung eines Verbindungstimeouts von mindestens 30 Sekunden. 
4. Testen Sie die Konnektivität zwischen dem Anwendungsserver und der Azure SQL-Datenbank, indem Sie [SQL Server Management Studio (SSMS) ](https://docs.microsoft.com/azure/sql-database/sql-database-connect-query-ssms), eine UDL-Datei, Ping und Telnet verwenden. Weitere Informationen finden Sie unter [Problembehandlung von SQL Server-Verbindungsproblemen](https://support.microsoft.com/help/4009936/solving-connectivity-errors-to-sql-server) und [Diagnose bei Verbindungsproblemen](https://docs.microsoft.com/azure/sql-database/sql-database-connectivity-issues#diagnostics).

   > [!NOTE]
   > Als Problembehandlungsschritt können Sie auch versuchen, die Konnektivität auf einem anderen Clientcomputer zu testen.

5. Stellen Sie als bewährte Methode sicher, dass Wiederholungslogik vorhanden ist. Weitere Informationen zur Wiederholungslogik finden Sie unter [Behandlung vorübergehender Fehler und Verbindungsfehler mit SQL-Datenbank](https://docs.microsoft.com/azure/sql-database/sql-database-connectivity-issues).

Wenn Ihr Problem mit den vorherigen Schritten nicht behoben werden kann, versuchen Sie, weitere Daten zu erfassen, und wenden Sie sich dann an den Support. Wenn es sich bei Ihrer Anwendung um einen Clouddienst handelt, aktivieren Sie die Protokollierung. Dieser Schritt gibt einen UTC-Zeitstempel des Fehlers zurück. Außerdem gibt SQL Azure die Ablaufverfolgungs-ID zurück. [Microsoft Customer Support Services](http://azure.microsoft.com/support/options/) kann diese Informationen verwenden. 
