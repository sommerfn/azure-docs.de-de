---
title: Problembehandlung bei der Diagnose der Azure SQL-Datenbank Hyperscale-Leistung | Microsoft-Dokumentation
description: In diesem Artikel wird beschrieben, wie Hyperscale-Leistungsprobleme in SQL-Datenbank behandelt werden.
...
ms.locfileid: "72392121"
---
# <a name="sql-hyperscale-performance-troubleshooting-diagnostics"></a>Diagnose zur Problembehandlung bei SQL-Hyperscale


Zur Behandlung von Leistungsproblemen in einer Hyperscale-Datenbank sind [Methodiken der allgemeinen Leistungsoptimierung](sql-database-monitor-tune-overview.md) auf dem Azure SQL-Datenbank-Computeknoten der Ausgangspunkt für eine Leistungsuntersuchung. Allerdings wurden angesichts der [verteilten Architektur](sql-database-service-tier-hyperscale.md) von Hyperscale zusätzliche Diagnosen zur Unterstützung hinzugefügt. In diesem Artikel werden Hyperscale-spezifische Diagnosedaten beschrieben.


## <a name="log-rate-throttling-waits"></a>Wartezeiten für Protokollratendrosselung


Bei jeder Azure SQL-Datenbank-Dienstebene gibt es Limits für die Protokollgenerierungsrate, die über die [Governance für die Protokollrate](sql-database-resource-limits-database-server.md#transaction-log-rate-governance) erzwungen werden. In Hyperscale ist das Limit für Protokollgenerierung – unabhängig von der Dienstebene – derzeit auf 100 MB/Sek. festgelegt. Es gibt jedoch Zeiten, in denen die Protokollgenerierungsrate für das primäre Computereplikat gedrosselt werden muss, um die SLAs (Vereinbarungen zum Servicelevel) für Wiederherstellbarkeit einzuhalten. Diese Drosselung erfolgt, wenn ein [Seitenserver oder ein anderes Computereplikat ](sql-database-service-tier-hyperscale.md) erheblich hinter der Anwendung neuer Protokolldatensätze vom Protokolldienst liegt.
...
|RBIO_RG_REPLICA        | Tritt auf, wenn eine Protokollgenerierungsrate für einen Hyperscale-Datenbank-Computeknoten infolge einer verzögerten Protokollnutzung durch das/den lesbaren sekundären Replikatknoten gedrosselt wird.         |
...
Wenn bei der Ausgabe eines Lesevorgangs in einem Computereplikat die Daten im Pufferpool oder lokalen RBEX-Cache nicht vorhanden sind, wird ein „getPage(pageId, LSN)“-Aufruf ausgegeben und die Seite vom entsprechenden Seitenserver abgerufen. Lesevorgänge von Seitenservern aus sind Remotelesevorgänge und folglich langsamer als Lesevorgänge aus dem lokalen RBPEX-Cache. Bei der Behandlung von E/A-bezogenen Leistungsproblemen müssen wir ermitteln können, wie viele E/A-Vorgänge über relativ langsamere Remoteseitenserver-Lesevorgänge erfolgten.

Wir haben Seitenserver-Lesevorgänge zu einem Satz von DMVs (Dynamic Management Views, dynamische Verwaltungssichten) und erweiterten Ereignissen hinzugefügt, um bei der Ermittlung zu helfen, welche Lesevorgänge Remotelesevorgänge von einem Seitenserver aus gegenüber logischen Lesevorgängen sind.
...
- „ActualPageServerReads/ActualPageServerReadAheads“ werden dem Abfrageplan-XML-Code für tatsächliche Pläne hinzugefügt.
...
- Bei der primären Compute-Instanz wird in „file_id 2“ von „sys.dm_io_virtual_file_stats“ ein Protokollschreibvorgang berücksichtigt. Ein Protokollschreibvorgang bei der primären Compute-Instanz ist ein Schreibvorgang in die Protokolllandezone, bei der es sich um ein Azure Storage Premium-Remoterepository handelt.
- Beim sekundären Replikat werden Protokolldatensätze dafür das bei einem Commit nicht festgeschrieben. Stattdessen wird das Protokoll wird vom Xlog-Dienst auf die Remotereplikate angewendet. Bestimmte Protokollschreibvorgänge erfolgen nicht tatsächlich in sekundären Replikaten und dienen nur zur Nachverfolgung.

## <a name="additional-resources"></a>Weitere Ressourcen

- Informationen zu V-Kern-Ressourcenlimits für eine Hyperscale-Einzeldatenbank finden Sie unter [Dienstebene „Hyperscale“ – V-Kern-Limits](sql-database-vcore-resource-limits-single-databases.md#hyperscale-service-tier-for-provisioned-compute).
